...
public class RoleController {
	...
	static {
		System.out.println("*************我他妈装摄像头RoleController********************");
	}
	
	@Resource
	private FunctionService functionService; //功能类的服务层
	...
	public String findFunctionAll(Model model,HttpServletResponse response,HttpServletRequest request){
		setPrivilegeFilterUser(request);
		try {
			List<Function> functions1 = functionService.findAll(); //这里查询处所有的功能
			System.out.println("难不成来到这了码");
			if(functions1==null){
	...
	public String findById(Integer id,HttpServletResponse response,HttpServletRequest request){
		setPrivilegeFilterUser(request);
		try {
			System.out.println("*************我他妈装摄像头********************");
			List<Object> obj = new ArrayList<Object>();
			Role role = this.roleService.findById(id);  //根据这个id查询处这个角色
			List<Function> functions1 = functionService.findAll(); //查询所有的功能
			obj.add(role);
